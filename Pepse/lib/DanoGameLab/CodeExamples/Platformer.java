...
import danogl.components.movement_schemes.MovementScheme;
...
class Avatar extends GameObject {
    ...
    public Avatar(Vector2 pos, UserInputListener inputListener) {
        ...
        movementScheme = new PlatformerMovementScheme(this, inputListener.keyboardMovementDirector());
    }

    @Override
    public void update(float deltaTime) {
        super.update(deltaTime);
        //space+down will ignore collisions for .5s, to go down a platform
        if(inputListener.isKeyPressed(KeyEvent.VK_SPACE) && inputListener.isKeyPressed(KeyEvent.VK_DOWN)) {
            ...
            //schedule turning collisions back on in .5s
            new ScheduledTask(this, .5f, false,
                    ()->physics().preventIntersectionsFromDirection(Vector2.ZERO));
            return;
        }
        //otherwise, use movementScheme.
        //if we didn't want the functionality of the last condition,
        // we could have just used addComponent(movementScheme)
        //in initialization and not override update.
