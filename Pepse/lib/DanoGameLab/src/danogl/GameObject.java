...
/**
 * Represents an object in the game. For example, an object has dimensions, a
 * certain position, it has a velocity, an image that represents it, and it can
 * potentially collide with other GameObjects.
 * 
 * @author Dan Nirel
 */
public class GameObject {
     ...
     *                      which case the GameObject will not be rendered.
     */
     ...
     * 
     * @param deltaTime The time elapsed, in seconds, since the last frame. Can be
     *                  used to determine a new position/velocity by multiplying
     *                  this delta with the velocity/acceleration respectively and
     *                  adding to the position/velocity: velocity +=
     *                  deltaTime*acceleration pos += deltaTime*velocity
     */
    public void update(float deltaTime) {
        transform.update(deltaTime);
        transform.setAccelerationEnabled(true); // might be cancelled when resolving
                                                // collisions
    ...
    /**
     * Renders the game object if the Renderable is not null. Should be called in
     * the rendering phase (i.e., not within an update). The method is automatically
     * called if the object was added to an active GameObjectCollection.
     * 
    ...
    public void render(Graphics2D g, Camera camera) {
        ...
        if (coordinateSpace != CoordinateSpace.WORLD_COORDINATES)
            throw new UnsupportedOperationException(
                    coordinateSpace + ": Unsupported coordinate space");
        ...
        freeCalculationsVector.setXY(getDimensions().x() * dimFactorX,
                getDimensions().y() * dimFactorY);
        renderer.render(g, camera.worldToScreenCoords(getTopLeftCorner()),
                freeCalculationsVector);
    }

    /**
     * Should this object be allowed to collide the the specified other object. If
     * both this object returns true for the other, and the other returns true for
     * this one, the collisions may occur when they overlap, meaning that their
     ...
     * @return true if the objects should collide. This does not guarantee a
     *         collision would actually collide if they overlap, since the other
     *         object has to confirm this one as well.
     */
     ...
     * @param collision Information regarding this collision. A reasonable elastic
    ...
    /**
     * Called on every frame of a collision with a given object, including the
     * first.
     * 
    ...
    /**
     * Set the top-left corner of this object. Moves the entire object so that its
     * top-left corner is as specified.
     */
    ...
    /**
     * Sets the object's width and height. Its center remains the same, while its
     * corners move.
     */
    ...
    /**
     * Gets the object's tag. The tag has no inherent meaning on its own and is not
     * used by the GameObject class itself; it can be assigned any meaning by the
     * user of the class, and is simply a convenience placeholder for custom info.
     */
    ...
    /**
     * Sets the object's tag. The tag has no inherent meaning on its own and is not
     * used by the GameObject class itself; it can be assigned any meaning by the
     * user of the class, and is simply a convenience placeholder for custom info.
     */
    ...
    /**
     * Add a component to the GameObject. A component includes a method that should
     * be executed on every update.
     */
    ...
    /**
     * remove a component from the GameObject. A component includes a method that
     * should be executed on every update.
     */
    ...
    /**
     * access to attributes concerning the object's transform (position, dimensions,
     * etc.)
     */
    ...
    /**
     * Gets the coordinate space to which the object's position is relative. The
     * default is world coordinates, but the parameter may specify that the object
     * is relative to the camera in which case it moves when the camera moves.
     */
    ...
    /**
     * Sets the coordinate space to which the object's position is relative. The
     * default is world coordinates, but the parameter may specify that the object
     * is relative to the camera in which case it moves when the camera moves.
     */
    ...
    /**
     * Translates world coordinates to object coordinates (relative to the top-left
     * corner and indifferent to dimensions).
     * 
    ...
    public Vector2 worldToObjectCoords(Vector2 worldCoords) {
        ...
        freeCalculationsVector.selfMultX(1f / getDimensions().x())
                .selfMultY(1f / getDimensions().y());
     ...
     * 
     * @param objectCoords a point relative to the top-left corner and indifferent
     *                     to dimensions
    ...
    public Vector2 objectToWorldCoords(Vector2 objectCoords) {
        ...
        freeCalculationsVector.selfMultX(getDimensions().x())
                .selfMultY(getDimensions().y()).selfAdd(getTopLeftCorner());
        return freeCalculationsVector;
    }
    ...
    /**
     * Invoked on the game object in case the mouse has been clicked, pressed,
     * released, or scrolled while the cursor is on top of the game object.
     * 
     * @param params info regarding the mouse's parameters in this frame and the
